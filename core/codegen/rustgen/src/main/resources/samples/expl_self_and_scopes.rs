...
/* operations */
		pub fn static_test(&mut self) -> i64 {
			let mut a: expl_self_and_scopes = self;
		let mut i: i64 = 123;
			expl_self_and_scopes::svoidOp();
		expl_self_and_scopes::svoidOp();
		expl_self_and_scopes::svoidOp();
		i = expl_self_and_scopes::sfiveop(1);
		return expl_self_and_scopes::sfiveop(2);
		...
		let mut i: i64 = 123;
		let mut a: expl_self_and_scopes = self;
			i = ::expl_self_and_scopes_cg_mod::testVal;
		self.voidOp();
		self.voidOp();
		i = self.fiveop(i);
		i = self.fiveop(x);
