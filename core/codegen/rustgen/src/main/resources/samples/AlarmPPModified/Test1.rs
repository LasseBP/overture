...
/* operations */
		...
		let  expert: ::Plant_cg_mod::Expert = self.plant.ExpertToPage(self.a1.clone(), ::Test1_cg_mod::p1.clone());
...
impl Default for Test1 {
	fn default() -> Test1 {
				let a1: ::Plant_cg_mod::Alarm = Alarm::new(strseq!("Mechanical fault"), ::Plant_cg_mod::Qualification::from(quotes::Mech));
				let a2: ::Plant_cg_mod::Alarm = Alarm::new(strseq!("Tank overflow"), ::Plant_cg_mod::Qualification::from(quotes::Chem));
				let ex1: ::Plant_cg_mod::Expert = Expert::new(set!(Any::from(quotes::Mech), Any::from(quotes::Bio)));
				let ex2: ::Plant_cg_mod::Expert = Expert::new(set!(quotes::Elec));
				let ex3: ::Plant_cg_mod::Expert = Expert::new(set!(Any::from(quotes::Chem), Any::from(quotes::Bio), Any::from(quotes::Mech)));
				let ex4: ::Plant_cg_mod::Expert = Expert::new(set!(Any::from(quotes::Elec), Any::from(quotes::Chem)));
