...
impl Plant {
    ...
    pub fn AddExpertToSchedule(&mut self, p: Period, ex: Expert) -> () {
        self.schedule.insert(p,
                             if self.schedule.domain().in_set(p.clone()) {
                                 set!(ex.clone()).union(self.schedule.get(p.clone()))
                             } else {
                                 set!(ex.clone())
                             });
    }
    ...
    pub fn RemoveExpertFromSchedule(&mut self, p: Period, ex: Expert) -> () {
        ...
        self.schedule = if exs.len() == 1 {
            self.schedule.dom_restrict_by(set!(p.clone()))
        } else {
            self.schedule.ovrride(map!(p.clone() => set!(ex.clone()).difference(exs.clone())))
        };
