...
/* operations */
	...
	return self.schedule.domain().set_compr(|p: Period| -> bool { self.schedule.get(p.clone()).in_set(ex.clone()) } , |p: Period| -> Period { p.clone() } );
		...
		self.schedule = sch.clone();
			...
			instance.cg_init_Plant_1(als, sch);
		return instance;
	
} 
	
				pub fn AddExpertToSchedule(&mut self, p: Period, ex: Expert) -> () {
	self.schedule.insert(p, if self.schedule.domain().in_set(p.clone()) { set!(ex.clone()).union(self.schedule.get(p.clone())) } else { set!(ex.clone()) });
} 
	
				pub fn RemoveExpertFromSchedule(&mut self, p: Period, ex: Expert) -> () {
			let  exs: Set<Expert> = self.schedule.get(p.clone());
			self.schedule = if exs.len() == 1 { self.schedule.dom_restrict_by(set!(p.clone())) } else { self.schedule.ovrride(map!(p.clone() => set!(ex.clone()).difference(exs.clone()))) };
